...
# Cumulative Water Deficit for one Gridpoint
...
The data wrangling started with reading the data and loading the required 
libraries.

- data format is netcdf,
- each variable needed is in a separate file and had therefore to be loaded separately
- grid is of global extent in climate format where there are just
positive values, (conversion conducted)
- resolution either daily or monthly in cases with monthly resolution a linear interpolation was conducted to obtain daily resolution
...
## Comparison CMIP6 evspsbl and calculated et
...
```
make t-test to see if they are statistically different
...
The null-hypothesis states in this
case, that the two data-sets have no significant difference. p value very small
rejection region. null-hypothesis rejected, there is a significant difference.
